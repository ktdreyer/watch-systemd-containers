...
def read_module_contents():
    with open('watch_systemd_containers/__init__.py') as app_init:
...
class PyTest(TestCommand):
    ...
    def run_tests(self):
        import pytest
        errno = pytest.main('watch_systemd_containers --flake8 ' +
                            self.pytest_args)
        sys.exit(errno)
...
setup(
    ...
    description='Watch containers running in systemd and restart them',
    packages=['watch_systemd_containers'],
    author='Ken Dreyer',
    author_email='kdreyer [at] redhat.com',
    url='https://github.com/ktdreyer/watch-systemd-containers',
    version=metadata['version'],
    license='MIT',
    zip_safe=False,
    keywords='docker, containers, systemd',
    long_description=LONG_DESCRIPTION,
    scripts=['bin/watch-systemd-containers'],
    install_requires=[
        'docker-py<=1.10.6',
    ],
